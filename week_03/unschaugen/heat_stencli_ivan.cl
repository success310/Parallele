...
//Read in local mem
        ...
        if(i_loc == 1 && i != 0)
            mem(a_loc,0,j_loc) = mem(A,i-1,j);
        else if (i_loc == 1 && i == 0)
            mem(a_loc,0,j_loc) = mem(A,i,j);

        if(j_loc == 1 && j != 0)
            mem(a_loc,i_loc,0) = mem(A,i,j-1);
        else if(j_loc == 1 && j == 0)
            mem(a_loc,i_loc,0) = mem(A,i,j);
...
//calc point
    ...
    value_t tl = mem(a_loc,i_loc,j_loc-1);
    value_t tr = mem(a_loc,i_loc,j_loc+1);
    value_t tu = mem(a_loc,i_loc-1,j_loc);
    value_t td = mem(a_loc,i_loc+1,j_loc);
